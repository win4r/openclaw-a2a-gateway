...
async function runAllTests() {
  console.log("=== Complete Connection Pool Tests ===\n");
  ...
  try {
    ...
    await test("Test 3: Create new connections when pool is not full", async () => {
      // Use different endpoints to avoid reuse
      const conn1 = await pool.acquire("http://localhost:18800");
      const conn2 = await pool.acquire("http://localhost:18801");
      const conn3 = await pool.acquire("http://localhost:18802");

      const stats = pool.getStats();
      console.log(`   Stats: total=${stats.totalConnections}, active=${stats.activeConnections}`);
      console.log(`   conn1.endpoint=${conn1.endpoint}, conn2.endpoint=${conn2.endpoint}, conn3.endpoint=${conn3.endpoint}`);
    ...
    // Test 4: Wait for available connection (事件驅動隊列)
    await test("Test 4: Wait for available connection (event-driven queue)", async () => {
      const endpoint = "http://localhost:18810"; // Use different port

      // Acquire max connections per endpoint (2)
      const conn1 = await pool.acquire(endpoint);
      const conn2 = await pool.acquire(endpoint);

      console.log(`   Acquired conn1: ${conn1.id}, conn2: ${conn2.id}`);
      console.log(`   Active: ${pool.getStats().activeConnections}, Total: ${pool.getStats().totalConnections}`);

      // Try to acquire a third connection (should wait)
      const conn3Promise = pool.acquire(endpoint);
      console.log(`   Waiting for third connection...`);

      // Check that it's waiting
      const statsWaiting = pool.getStats();
      console.log(`   Waiting requests: ${statsWaiting.waitingRequests}`);
      assert(statsWaiting.waitingRequests === 1, "Should have 1 waiting request");
      ...
      console.log(`   Received conn3: ${conn3.id}`);
    ...
    await test("Test 5: Per-endpoint limit", async () => {
      ...
      console.log(`   Endpoint ${endpoint}: ${pool.getStats().totalConnections} total`);
